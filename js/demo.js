function getFECLevel() {
    // forward correction error level (0=low 3=max)
    for(const x of Array(4).keys()) {
        if(document.getElementById("radio-quality-"+x).checked) {
            return x;
        }
    }
}
...
function getCRC() {
    // add 16 bits crc if != 0
    return document.getElementById("radio-crc-on").checked;
}
...
function sendChat() {
    ...
    tx.transmit(bytes, getFECLevel(), getCRC());
}

function sendColor() {
    // Send data
    var tx = QRTone.transmitter({onFinish: function () { console.log("transmission complete"); }});
    tx.transmit([colorPicker.color.rgb.r, colorPicker.color.rgb.g, colorPicker.color.rgb.b], getFECLevel(), getCRC());
}
...
function resetMsg() {
    $("#receivedMessage")[0].value = "Click on \"Mic On\" in order to listen for messages.\n";
}
window.setTimeout(resetMsg, 500);
